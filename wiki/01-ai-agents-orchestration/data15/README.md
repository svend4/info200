...
- Issues: 0; звёзды/форки: 0; закрытых PR — 4. В истории есть коммит «SECURITY: remove hardcoded MiniMax API key» — ключ убран в переменную окружения (но, вероятно, остался в git-истории — не проверено).
...
| Размер | 31 файл кода/доков в корне + ~50 файлов в `docs/`, `orchestrator/` и `user_input_files/`; ~15 000 строк ключевых файлов, из них 6 579 — `orchestrator_v5.py` |
