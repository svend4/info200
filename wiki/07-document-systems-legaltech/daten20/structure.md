...
├── AUDIT_REPORT.md             # Самоаудит от 2026-01-11 (183 py-файла / 89 176 строк Python; 127 710 строк во всех 234+ файлах)
...
├── src/                        # 72 директории — см. ниже
├── tests/                      # ~80 test_*.py (плоско, зеркалят src/) + ~18 поддиректорий: unit/ integration/ e2e/ performance/ fixtures/ core/ ai/ security/ microservices/ iot/ …
...
## Слои src/ (72 директории — пересчитано)
...
| `.github/workflows/tests.yml` | CI: Python 3.9/3.10/3.11, но запускает **только** `pytest tests/test_doc_*.py` — 7 файлов из ~80 | Ключ к пониманию «зелёного» бейджа тестов |
...
4. **Разросшийся плоский `tests/`**: ~80 файлов `test_*.py` свалены плоско в один каталог `tests/` (плюс ~18 поддиректорий, дублирующих структуру `src/`) — почти по тесту на каждый модуль, включая AI-надстройку (`test_consciousness.py`, `test_quantum_ml.py`, `test_asi_beyond_human.py`, `test_cosmic_universal.py`). При этом CI (`tests.yml`) запускает лишь 7 из них (`test_doc_*.py`); остальные ~73 в CI не проверяются. (Перепроверено адверсариально: embedded-тестов **внутри** `src/` нет — заявление предыдущего аудита про «`test_*.py` в `src/bci/`» не подтвердилось, файлы `src/bci/test_*.py` дают 404; все тесты собраны в `tests/`.)
