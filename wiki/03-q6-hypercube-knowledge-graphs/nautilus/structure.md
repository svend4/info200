...
│   └── «Вакансии в Anthropic по кластерам» ×4 файла (без расширения + .json/.md/.txt)  # ПОСТОРОННИЙ контент в docs/
│
...
- Небольшой мусор: в `docs/` лежат посторонние файлы («Вакансии в Anthropic по кластерам» — четыре файла: экспорт без расширения плюс `.json/.md/.txt`), не относящиеся к порталу.
