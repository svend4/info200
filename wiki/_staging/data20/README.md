...
Парадокс проекта: при огромном объёме инфраструктуры сама база знаний почти пуста — в ней порядка 4–5 статей в трёх категориях (компьютеры, быт, кулинария). Это «завод по производству знаний», построенный до того, как появилось сырьё.
...
CI/CD: 12 workflow в `.github/workflows/` — тесты с coverage (pytest, матрица Python 3.10/3.11, Codecov), валидация базы знаний, 5+ вариантов сборки Android APK, release-автоматизация.
...
| Контент базы знаний | всего ~4–5 статей в 3 категориях (почти пусто) |
| Последняя активность | 2026-01-08 (374 коммита за неделю 02–08.01.2026) |
| Релизы | v1.0.0 (04.01.2026), без APK в ассетах |
| CI | 12 workflow, 477 запусков; тесты pytest + coverage + Codecov |
