...
│
├── requirements.txt              # flask>=2.3.0 (фактически не нужен: API на stdlib), остальное закомментировано
...
│
└── user_input_files/             # Входные материалы автора (сессии анализа, docx)
...
3. **Runtime-состояние в git**: каталоги `orchestrator/state/`, `tasks/`, `cache/` с рабочими JSON (включая `users.json` с хэшами паролей и тестовыми записями кэша) закоммичены.
4. **Следы агентной песочницы**: жёсткие пути `/workspace/orchestrator` в вспомогательных файлах (`hermes_llm.py`, `monitor_daemon.py`, `stress_test.py`, `hybrid_orchestrator.py`, `openclaw_integration.py`); сам `orchestrator_v5.py` уже переведён на относительные пути `Path(__file__).parent`.
