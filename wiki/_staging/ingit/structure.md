...
│   │   ├── api/
│   │   │   ├── health.py         # /api/health, /api/ping — единственные работающие эндпоинты
│   │   │   ├── projects.py       # CRUD-стабы: TODO, возвращают []/501/404
│   │   │   ├── tasks.py          # CRUD-стабы + assign; всё TODO
│   │   │   └── documents.py      # CRUD-стабы; всё TODO
│   │   ├── cli/
│   │   │   └── main.py           # 349 строк, click: ingit init / task create / task list / report
...
| `backend/ingit/api/*.py` | REST-эндпоинты projects/tasks/documents | Стабы (TODO/501/404); реально работают только /health и /ping |
| `backend/ingit/cli/main.py` | CLI: init (структура 00–90), task create/list, report | Автономен от БД, выглядит работоспособным (не запускался — не проверено) |
