...
│   ├── 07_DOMAIN_PACKS/             # Пустой каркас следующего слоя (Gov/Healthcare/Education): только README с «Blocks: 0», в _block_index.json не входит
│   ├── NEXT-17…NEXT-20, NEXT-22/    # Legacy-формат января 2026 (NEXT-21 отсутствует): README/spec/manifest/checksums + pack_files
...
4. **Дыры и дубли в нумерации** (пересчёт `_block_index.json` в рамках этого аудита): всего **8 дыр** — 5 известных проекту (14241–14440 битый ZIP, 21201–22000, 26001–26400, 30401–30800, 31201–31600) плюс **3 нигде не задокументированные** (49201–49600, 80001–80100, 81301–82000); и **30 задублированных диапазонов** (60 блоков попарно делят один диапазон шагов, в основном в 04_ADVANCED и 05_ENTERPRISE), тогда как в «Известных проблемах» мастер-индекса признан только один дубль — 35601–36000 (developer_platform и identity_profiles). Сама очередь честно держит задачу «Проверить пробелы в нумерации» в MEDIUM.
5. **Несоответствия README реальности**: заявленная папка `releases/` не существует; `README_IFOS_20260113.md` называет папку `master-index/` (через дефис), фактически она `master_index/`; ссылки мастер-индекса на `./00_GENESIS/README.md` и другие README слоёв 00–06 битые (файлов нет) — единственный README слоя есть у пустого `07_DOMAIN_PACKS/`.

---

[← Обзор](README.md) · [Характеристика и оценка](assessment.md) · [Рекомендации](recommendations.md) · [Вопросы](questions.md)
