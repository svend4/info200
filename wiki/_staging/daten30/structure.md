...
│   ├── FINAL_SUMMARY.md             # САМООТЧЁТ: «5 вариантов завершены» (перепроверен, см. assessment)
...
| `.github/workflows/test-backend.yml` | Матрица Python 3.9–3.11; фактически только `python -m py_compile` + запуск Registry и `curl /health`, юнит-тестов нет (pytest ставится, но «pytest not found, skipping tests») |
...
- **Документация >> код**: на каждый компонент приходится по несколько подробных русскоязычных MD; корень репозитория — это фактически учебник.
