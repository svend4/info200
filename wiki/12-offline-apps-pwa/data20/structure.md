...
```
data20/
├── knowledge/                  # СОБСТВЕННО БАЗА ЗНАНИЙ (почти пустая: ровно 3 статьи)
│   ├── computers/              # ИТ-категория
│   │   ├── index/INDEX.md
│   │   └── articles/
│   │       ├── ai/             # 1 файл: llm-overview-2026.md (frontmatter повреждён — см. assessment)
│   │       └── programming/    # 1 файл: python-patterns.md (frontmatter тоже повреждён)
│   ├── household/              # быт: articles/appliances/refrigerator-buying-guide-2026.md + index
│   └── cooking/                # только index/INDEX.md, статей нет («раздел в разработке»)
│
...
│
├── .github/workflows/          # 10 файлов CI/CD: tests.yml (pytest+coverage+Codecov, Py 3.10/3.11),
