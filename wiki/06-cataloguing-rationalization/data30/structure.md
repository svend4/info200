...
Итого: ~85 файлов в 13 каталогах верхнего уровня (`catalog`, `data`, `docs`, `migrations`, `plans`, `prototype`, `qa`, `readmes`, `schemas`, `scripts`, `src`, `templates`, `tests`) плюс 8 корневых .md-документов; точное число файлов не проверено — листинги GitHub обрезают длинные списки.
...
| `docs/api/openapi.yaml` | Контракт REST API v1 (731 строка): CRUD functions/modules/use-cases/macros/reviews/ratings + эндпоинты `/{id}/history` для истории и отката. Аутентификация в контракте НЕ описана: `components` содержит только `parameters` и `schemas`, блоков `securitySchemes`/`security` нет (проверено парсером). |
